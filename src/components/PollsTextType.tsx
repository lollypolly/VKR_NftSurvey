...
function PollsTextType({ number, exportCallback, text }: any) {
  ...
  const addAnswerOneVariant = () => {
    if (answersOneVariant.length < 4) {
      const newAnswers: any = [...answersOneVariant, ""];
      setAnswersOneVariant(newAnswers);
    }
  };
  const deleteAnswerOneVariant = () => {
    const updatedItems = answersOneVariant.filter(
      (index: any, i: any) => i !== +index
    );
    setAnswersOneVariant(updatedItems);
  };
    ...
    <>
      <div className="poll-input-container">
        <div className="poll-input-header">
          ...
          <Input
            type="text"
            value={questionOneVariant}
            onChange={handleQuestionOneVariantChange}
          />
        </div>
      </div>
      <div className="poll-input-container">
        ...
        <button
          className="poll-button-bold question-button-bold"
          onClick={addAnswerOneVariant}
        >
          Добавить вариант
        </button>
      </div>
