...
function Poll ({name,el}:any) {
    ...
    const onGetQuestions = () =>
    {
        ...
        getUsersAnswers(
            ...
            if (!res.length) {
                console.log("resultat-length",res)
                navigate('/answers')
            } else {
                console.log("resultat-length else",res)
    ...
    return (
        <div className="poll-container" onClick={onGetQuestions}>
            ...
            <div className="poll-container-right">
                <div className={el.creator===account?"blue-side": "blue-side-none"}>{el.creator===account?'Вы создатель опроса': ""}</div>
